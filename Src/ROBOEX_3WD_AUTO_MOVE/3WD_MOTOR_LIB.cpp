...
void Motor_DIR_speed_setup(uint8_t DIR,uint8_t SPEED, uint8_t dist)
{
	...
	{
		case 1:
			PID_MOTOR[0].PWM_default = 0;
			PID_MOTOR[1].PWM_default = speed * SPEED * 1.732/2 * 1.04;
			PID_MOTOR[2].PWM_default = PID_MOTOR[1].PWM_default;
			Motor_DIR(num1,STOP);
			Motor_DIR(num2,CCW);
			Motor_DIR(num3,CW);
			break;
		case 2:
			...
			break;
		case 3:
			PID_MOTOR[0].PWM_default = speed * SPEED * 1.04;
			PID_MOTOR[1].PWM_default = PID_MOTOR[0].PWM_default;
			PID_MOTOR[2].PWM_default = PID_MOTOR[1].PWM_default;
			Move_default[0] = dist * 82.76;
			Move_default[1] = Move_default[0];
			Move_default[2] = Move_default[1];
			Encoder_flag = 1;
			Motor_DIR(num1,CW);
			Motor_DIR(num2,CW);
			Motor_DIR(num3,CW);
